moci_ampute_t15 <- rep(0, length(proporcije))
moci_impute_mean_t15 <- rep(0, length(proporcije))
moci_impute_svd_t15 <- rep(0, length(proporcije))
moci_impute_3nn_t15 <- rep(0, length(proporcije))
moci_impute_6nn_t15 <- rep(0, length(proporcije))
moci_impute_10nn_t15 <- rep(0, length(proporcije))
...
for (i in 1:length(proporcije)) 
{
  ...
  moci_ampute_t15[i] <- moc_testa(nulte1[[n_new]], T2, 0.05)
  
  moci_impute_mean_t15[i] <- moc_testa(T, T1_mean, alpha_calib_mean[i])
  moci_impute_svd_t15[i] <- moc_testa(T, T1_svd, alpha_calib_svd[i])
  moci_impute_3nn_t15[i] <- moc_testa(T, T1_3nn, alpha_calib_3nn[i])
  moci_impute_6nn_t15[i] <- moc_testa(T, T1_6nn, alpha_calib_6nn[i])
  moci_impute_10nn_t15[i] <- moc_testa(T, T1_10nn, alpha_calib_10nn[i])
...
df_mean$"Моћи" <- moci_impute_mean_t15
df_mean$"Метод" <- "Mean"
...
df_svd$"Моћи" <- moci_impute_svd_t15
df_svd$"Метод" <- "SVD"
...
df_3nn$"Моћи" <- moci_impute_3nn_t15
df_3nn$"Метод" <- "3NN"
...
df_6nn$"Моћи" <- moci_impute_6nn_t15
df_6nn$"Метод" <- "6NN"
...
df_10nn$"Моћи" <- moci_impute_10nn_t15
df_10nn$"Метод" <- "10NN"
...
tikz(file = "plot_moci_t15_stdn.tex", width = 5, height = 3, Encoding(UTF-8))
...
ggplot(data = df, aes(x = df[, 1], y = df[, 2], group = df[, 3], colour = df[, 3]))+
  ...
  ylab("Moc")+
  labs(col="Метод")+
  ggtitle("Standardna t15 alternativa")
