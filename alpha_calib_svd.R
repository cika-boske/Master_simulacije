...
alpha_calib_svd <- rep(0, length(proporcije))
...
vracene_moci_svd <- rep(0, length(alpha_calib_svd))
...
for (i in 1:length(proporcije))
{
  ...
  for (j in 1:N)
  {
    ...
    # vrsimo imputaciju
    uzorak_ubaceno <- eimpute(uzorak_skalirano, r=1)$x.imp
    # reskaliramo
  ...
  alpha_calib_svd[i] <- alpha_probno[indeksi_min]
  # Zbog kontrole da je zaista vraceno 0.05
  vracene_moci_svd[i] <- moc_testa(T, T_ubaceno, alpha_calib_svd[i])
}