...
alpha_calib_cart <- rep(0, length(proporcije))
...
vracene_moci_cart <- rep(0, length(alpha_calib_cart))
...
for (i in 1:length(proporcije))
{
  ...
  for (j in 1:N)
  {
    ...
    # vrsimo imputaciju
    uzorak_ubaceno <- data.matrix(complete(mice(uzorak_skalirano, m = 1, method = "cart")))
    # reskaliramo
  ...
  alpha_calib_cart[i] <- alpha_probno[indeksi_min]
  # Zbog kontrole da je zaista vraceno 0.05
  vracene_moci_cart[i] <- moc_testa(T, T_ubaceno, alpha_calib_cart[i])
}