...
alpha_calib_3nn <- rep(0, length(proporcije))
...
vracene_moci_3nn <- rep(0, length(alpha_calib_3nn))
...
for (i in 1:length(proporcije))
{
  ...
  alpha_calib_3nn[i] <- alpha_probno[indeksi_min]
  # Zbog kontrole da je zaista vraceno 0.05
  vracene_moci_3nn[i] <- moc_testa(T, T_ubaceno, alpha_calib_3nn[i])
}