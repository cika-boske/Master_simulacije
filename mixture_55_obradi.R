moci_ampute_mixture55 <- rep(0, length(proporcije))
moci_impute_mean_mixture55 <- rep(0, length(proporcije))
moci_impute_svd_mixture55 <- rep(0, length(proporcije))
moci_impute_3nn_mixture55 <- rep(0, length(proporcije))
moci_impute_6nn_mixture55 <- rep(0, length(proporcije))
moci_impute_10nn_mixture55 <- rep(0, length(proporcije))
moci_impute_pmm_mixture55 <- rep(0, length(proporcije))
moci_impute_cart_mixture55 <- rep(0, length(proporcije))
...
for (i in 1:length(proporcije)) 
{
  ...
  for (j in 1:N) 
  {
    uzorak <- rMVNmixture(n,
                          weight = c(0.3, 0.7),
                          mean = matrix(c(0, 0, 5, 5), nrow = 2, byrow = TRUE),
                          Sigma = sigme)
  ...
  moci_ampute_mixture55[i] <- moc_testa(nulte1[[n_new]], T2, 0.05)
  
  moci_impute_mean_mixture55[i] <- moc_testa(T, T1_mean, alpha_calib_mean[i])
  moci_impute_svd_mixture55[i] <- moc_testa(T, T1_svd, alpha_calib_svd[i])
  moci_impute_3nn_mixture55[i] <- moc_testa(T, T1_3nn, alpha_calib_3nn[i])
  moci_impute_6nn_mixture55[i] <- moc_testa(T, T1_6nn, alpha_calib_6nn[i])
  moci_impute_10nn_mixture55[i] <- moc_testa(T, T1_10nn, alpha_calib_10nn[i])
  moci_impute_cart_mixture55[i] <- moc_testa(T, T1_cart, alpha_calib_cart[i])
  moci_impute_pmm_mixture55[i] <- moc_testa(T, T1_pmm, alpha_calib_pmm[i])
...
df_amp$"Моћи" <- moci_ampute_mixture55
df_amp$"Метод" <- "CC"
...
df_mean$"Моћи" <- moci_impute_mean_mixture55
df_mean$"Метод" <- "Mean"
...
df_svd$"Моћи" <- moci_impute_svd_mixture55
df_svd$"Метод" <- "SVD"
...
df_3nn$"Моћи" <- moci_impute_3nn_mixture55
df_3nn$"Метод" <- "3NN"
...
df_6nn$"Моћи" <- moci_impute_6nn_mixture55
df_6nn$"Метод" <- "6NN"
...
df_10nn$"Моћи" <- moci_impute_10nn_mixture55
df_10nn$"Метод" <- "10NN"
...
df_pmm$"Моћи" <- moci_impute_pmm_mixture55
df_pmm$"Метод" <- "PMM"
...
df_cart$"Моћи" <- moci_impute_cart_mixture55
df_cart$"Метод" <- "CART"
...
tikz(file = "plot_moci_mixture55_norm1.tex", width = 5, height = 3, Encoding(UTF-8))
...
ggplot(data = df, aes(x = df[, 1], y = df[, 2], group = df[, 3], colour = df[, 3]))+
  ...
  ylab("Моћи")+
  labs(col="Метод")+
  ggtitle("Стандардна mixture55 алтернатива")
