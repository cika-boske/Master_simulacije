...
alpha_calib_mean <- rep(0, length(proporcije))
...
vracene_moci_mean <- rep(0, length(alpha_calib_mean))
...
for (i in 1:length(proporcije))
{
  ...
  for (j in 1:N)
  {
    ...
    # vrsimo imputaciju
    uzorak_ubaceno <- impute_mean(uzorak_skalirano, type = "columnwise")
    # reskaliramo
  ...
  alpha_calib_mean[i] <- alpha_probno[indeksi_min]
  # Zbog kontrole da je zaista vraceno 0.05
  vracene_moci_mean[i] <- moc_testa(T, T_ubaceno, alpha_calib_mean[i])
}